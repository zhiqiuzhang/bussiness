...
class Contact extends Component{
	...
	defaultContact(id){
		api.defaultContact(id).then((res)=>{
			if(res.OK){
				...
				message.error("设为默认收货人地址 失败"+res.message);
			}
	...
	render(){
		const {showModal, action, allContacts, defaultValues, moreBtn}=this.state;
		let okModal;
		let title;
		if(action==="new"){
			title="新增收货人地址";
		...
		return (
			<div className={"contact"}>
				<h3>收货人信息</h3>
				<Button className="new" size="small" onClick={()=>this.setState({showModal: true, action: "new",defaultValues: {}})}>新增收货人地址</Button>
				...
				<Button size="small" onClick={()=>this.setState({moreBtn: !this.state.moreBtn})}>
